<div>
    ...
    <head>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css"
            integrity="sha384-ezgG2veqOA/5zPBnKFVr+1tFZZHoOjHshEWbRl0Yt83QCBTZw1A+DIADKh5F9UOI" crossorigin="anonymous">
    ...
    <div style="margin-top:5px;margin-bottom:5px">
        <!-- ... other buttons ... -->
        <input wire:change="filter" wire:model="search" type="text" placeholder="Search employees"
        style="width: 200px; border-radius: 5px; padding: 5px; margin-top: 10px; margin-left: 10px;">
                    ...
                    @foreach ($employees as $employee)
                        <tr>
                            ...
                            <td class="whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($employee->date_of_birth)->format('Y M d') }}</td>
                            <td class="whitespace-nowrap">{{ $employee->mobile_number }}</td>
                            <td class="whitespace-nowrap">
                                {{ ucwords(str_replace(['-', '_'], ' ', $employee->employee_type)) }}</td>
                            <td class="whitespace-nowrap">
                                        ...
                                        <div class="d-inline-block">
                                            @if ($employee->status == 1)
                                                <button class="btn btn-danger btn-xs"
                                                    wire:click="deleteEmp('{{ $employee->emp_id }}')">
